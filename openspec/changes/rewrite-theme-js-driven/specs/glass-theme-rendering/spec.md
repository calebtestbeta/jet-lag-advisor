...
系統 SHALL 確保 `.lg-glass` 卡片在日間模式（`isDark === false`，目的地 `timeSegment` 為 `awake` 或 `dawn`）下顯示高亮白色半透明外觀，不使用 `backdrop-filter`，不受裝置系統主題影響。**主題值由 JavaScript 直接注入 `#app` inline style，不依賴 CSS class cascade。**
...
系統 SHALL 確保「航程管理」卡片中的時間輸入框有可辨識背景：日間為白色（R channel ≥ 200），夜間為深色（R channel < 80）。**`color-scheme` 仍由 `theme-day` / `theme-night` class 控制，輸入框背景由 `--glass-input-bg` token 透過 JS 注入。**
